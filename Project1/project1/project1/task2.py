...
def norm_xcorr2d(patch, template):
    ...
    temp_sum = 0
    patch_sum = 0
    # print("len",len(patch[:]),len(patch[0]))
    # print(template)
    for i in range(24):
        for j in range(26):
            temp_sum += template[i][j]
            patch_sum += patch[i][j]
    temp_mean = temp_sum/(24*26)
    patch_mean = patch_sum/(24*26)
    ...
    for a in range(24):
        for b in range(26):
            sum1 += (template[a][b] - temp_mean) * (patch[a][b] - patch_mean)
            sum2 += (template[a][b] - temp_mean) ** 2
            sum3 += (patch[a][b] - patch_mean) ** 2

    ncc = sum1/((sum2*sum3) ** 0.5)
    # print(ncc)
    return ncc
...
def match(img, template):
    ...
    temp_x = len(template[:])#24
    temp_y = len(template[0])#26

    max_value = 0
    times = 0
    
    for i in range(x_len-temp_x):
        for j in range(y_len-temp_y):
            patch = [m[j:j+temp_y] for m in img[i:i+temp_x]]
            maxii = norm_xcorr2d(patch,template)
            if (maxii > max_value):
                max_value = maxii
                x = i
                y = j
