...
def convolve2d(img, kernel):
    ...
    """
    kernel = utils.flip2d(kernel,1)
    old_len = len(img)
    #zeropad
    flipped_img = utils.zero_pad(img,1,1)
    i = 0
    j = 0

    new_len = len(flipped_img)
    new_img = [[0 for x in range(new_len)] for y in range(new_len)]
   
    for i in range(old_len):
        for j in range(old_len):
            # print(flipped_img[20][40])
            new_img[i][j] = flipped_img[i][j] * kernel[0][0] + flipped_img[i][j+1] * kernel[0][1] + flipped_img[i][j+2] * kernel[0][2] 
            + flipped_img[i+1][j] * kernel[1][0] + flipped_img[i+1][j+1] * kernel[1][1] + flipped_img[i+1][j+2] * kernel[1][2] 
            + flipped_img[i+2][j] * kernel[2][0] + flipped_img[i+2][j+1] * kernel[2][1] + flipped_img[i+2][j+2] * kernel[2][2]
